...
class EvCBR:
    ...
    def forecast_effects(self, *,
        ...
        print("DUMMY TARGET URI:"+str(dummy_target_uri))
        print("##############################################")
        ...
        print("#########################")
        print("InductiveRelations: "+str(inductive_input_relations))
        print("Total Case count:"+str(total_case_count))
        print("###########################")
        ...
        print("Similar Cause Effects: "+str(similar_cause_effects))
        print("LENGTH OF SIMILAR CAUSE EFFECTS: "+str(len(similar_cause_effects)))
        print("original_sc_count: "+str(original_sc_count))
        ...
        for sc in sim_ce:
            ...
            ###########################
            print("######################")
            print("SC_CAUSE_URI: "+str(sc_cause_uri))
            print("SC_EFFECT_URI: "+str(sc_effect_uri))
            print("Relevant cause first_step_vals")
            print(str(relevant_cause_firststep_vals))
            print("#############################")
            print("SC_EFFECT_TARGETS")
            print(str(sc_effect_targets))
            print("##############################")
            ##############################
            ...
            if get_cycles:
                smallcycles = self.fast_get_and_format_cycles(target_uri=sc_cause_uri,
                                                              ...
                                                              max_hops=max_hops)
                print("#######################")
                print("SMALL CYCLE: "+str(smallcycles))
                print("########################")
            ...
            for prop, paths in prop_paths.items():
                ...
                print("###################################")
                print("ALL PATH EDGES")
                print(str(all_path_edges))
                print("###########################")
                ...
                for path_edges in all_path_edges:
                    ...
                    if (sc_cause_uri, tuple(path_edges)) in res_cache.keys():
                        ...
                        print("#####################################")
                        print("sc_cause_uri:"+str(sc_cause_uri))
                        print("path_edges: "+str(tuple(path_edges)))
                        print("PATH FORECASTS")
                        print(str(path_forecasts))                        
                        print("#####################################")
                    else:
                        ...
                        print("########################################")
                        print("IN ELSE: PATH FORECAST: "+str(path_forecasts))
                        print("SC_CAUSE URI: "+str(sc_cause_uri))
                        print("######################################")
                ...
                for path_edges in set(all_path_edges):
                    if miniprop_totals[path_edges] > 0:
                        print("####################################")
                        print("miniprop_totals: path edges: "+str(path_edges))
                        print("miniprop_totals: "+str(miniprop_totals[path_edges]))
                        print("Path counts: "+str(miniprop_counts[path_edges]))
                        print("miniprop_successs[pathedges]: "+str(miniprop_successes[path_edges]))
                        print("PROP: "+str(prop))
                        print("miniprop_successes[path_edges: "+str(miniprop_successes[path_edges]))
                        print("miniprop_totals[path_edges]: "+str(miniprop_totals[path_edges]))
                        print("SMOOTHING_CONSTANT: "+str(SMOOTHING_CONSTANT))
                        print("miniprop_successes[path_edges]/(miniprop_totals[path_edges]+SMOOTHING_CONSTANT)): "+str(miniprop_successes[path_edges]/(miniprop_totals[path_edges]+SMOOTHING_CONSTANT)))
                        print("###################################")
        ...
        for p, ps in prop_path_stats.items():
            ...
            for k in ps['path_successful_results'].keys():
                print("################################")
                print("ps['path_precision: K:"+str(k))
                print("(ps['path_successful_results'][k]:" +str((ps['path_successful_results'][k])))
                print("ps['path_total_results'][k]:"+str(ps['path_total_results'][k]))
                print("(ps['path_successful_results'][k]/(ps['path_total_results'][k]+SMOOTHING_CONSTANT)):"+str((ps['path_successful_results'][k]/(ps['path_total_results'][k]+SMOOTHING_CONSTANT))))
                print("###################################################")
        ...
        for p, ps in prop_path_stats.items():
            ...
            for path, precision in best_paths:
                ...
                print("############################################")
                print("PATH: "+str(path))
                print("PATH FORECast: "+str(path_forecast))
                print("PATH PRECISION: "+str(precision))
                print("##############################################")
                if path_forecast:
                    valid_path_count += 1
                    print("#################################")
                    print("path: "+str(path))
                    print("dummy_target_uri: "+str(dummy_target_uri))
                    print("#######################################")
                for forecast_ent, forecast_ent_count in path_forecast.items():
                    print("###########################################")
                    print("Forecast Ent: "+str(forecast_ent))
                    print("Forecast_en_count:"+str(forecast_ent_count))
    ...
    def refine_similar_cases_with_target_forecasts(
        ...
        print('=====================================================')
        print("Information for refine similar case with target forecast")
        print('================================')
        print('target entity: '+str(target_entity))
        print('candidates: '+str(candidates))
        print('forecast_properties: '+str(forecast_properties))
        print('forecast_step_relation: '+str(forecast_step_relation))
        print('=========================================')
        ...
        for p in target_outgoing.keys():
            ...
            print('associate objects\n=======================')
            print('target_outgoing[p]:'+str(target_outgoing[p]))
            obj_weight = dict()
            for o in target_outgoing[p]:
                obj_occ = len(list(self.KG.subject_predicates(object=o)))
                print('o: '+str(o)+" and its occurences in KG is obj_occ: "+str(obj_occ))
                match_count = len(list(self.KG.subjects(predicate=p, object=o)))
                print('match count (between predicate: '+str(p)+' and object: '+str(o)+' match_count: '+str(match_count))
                # set weight as the log of P(relation p given object o) / P(triples incoming to o in the KG)
                weight = np.log10((match_count/obj_occ) / (obj_occ/self.total_triples))
                print('weight caclcuated: np.log10((match_count: '+str(match_count) +'/obj_occ: '+str(obj_occ) +')/(obj_occ:'+str(obj_occ)+"/self.total_triples:"+str(self.total_triples)+")):")
                obj_weight[o] = weight
                print('obj_weight:'+str(obj_weight[o]))
                max_pred_weight = max(max_pred_weight, weight)
            # each predicate weight differs based on the outgoing node it matches to.
            # this is only relevant if there are multiple outgoing relations for the forecast.
            sim_weight[p] = obj_weight
        ...
        for c in candidates:
            ...
            for p,o in self.KG.predicate_objects(subject=c):
                candidate_outgoing[p].add(o)
            total_score = 0
            print("candidate Ougoing: "+str(candidate_outgoing))
            for k, sims in target_outgoing_sims.items():
                max_sim = 0
                for cand_out_ent in candidate_outgoing[k]:
                    if cand_out_ent in self.e2i.keys():
                        print('Cand_out_ent:'+str(cand_out_ent))
                        for ent, sim in sims.items():
                            print("ENT: "+str(ent)+"\tsim:"+str(sim))
                            max_sim = max(max_sim, sim_weight[k][ent]*sim[0, self.e2i[cand_out_ent]])
                total_score += max_sim
                candidate_score_breakdown[c].append(max_sim)
            best_outgoing_sim = 0
            ...
            for outgoing_forecast_ent in self.KG.objects(subject=c, predicate=forecast_step_relation):
                ...
                if this_outgoing_sim > best_outgoing_sim:
                    ...
                    print("C: "+c)
                    print("OUTGOING_FORECAST_ENT: "+str(outgoing_forecast_ent))
                    print("OUGOING_RELATIONS: "+str(outgoing_relations))
                    print("COMPUTING JACARD SIMILARITY")
                    print("OUTGOING_RELATION_INTERSECTION: "+str(outgoing_relations.intersection(forecast_properties)))
                    print("OUTGOING_RELATION_UNION: "+str(outgoing_relations.union(forecast_properties)))
                    print("THIS_OUT_GOING_SIM: "+str(this_outgoing_sim)) 
        ...
        sorted_candidates = [tup[0] for tup in sorted_candidates]
        print('===================================')
        print('output information of refine similar cases with target forecasts')
        print('-----------------------------------------------')
        print('sorted_candidates: '+str(sorted_candidates))
        print('sc_scores: '+str(sc_scores))
        print('case_connection_scores: '+str(case_connection_scores))
        print('-----------------------------------------------')
    ...
    def refine_similar_cases_with_target_forecasts_effect_coverage(
        ...
        # to get the similarity of each candidate.
        print('===========================================')
        print('Information regarding input for refine_similar_cases with forecasts_effect_coverage')
        print('--------------------------------------------------------')
        print('target entity: '+str(target_entity))
        print('candidates: '+str(candidates))
        print('forecast_properties: '+str(forecast_properties))
        print('forecast_step_relation:'+str(forecast_step_relation))
        print('selected_cases: '+str(selected_cases))
        print('=========================================')
        
        target_outgoing = defaultdict(lambda: set())
        for p,o in self.KG.predicate_objects(subject=target_entity):
            target_outgoing[p].add(o)
            print('target entity: '+str(target_entity)+"\t target_outgoing: "+str(p)+"\t target outgoing_object:"+str(o))
        ...
        for k, v in target_outgoing.items():
            for ent in v:
                target_outgoing_sims[k][ent] = weighted_sim[out_to_id[ent]]
        print('=======================')
        print('target_outgoing_sim[k][ent]')
        print(target_outgoing_sims)
        print('========================')
        ...
        for c in candidates:
            ...
            for p,o in self.KG.predicate_objects(subject=c):
                candidate_outgoing[p].add(o)
                print('candidate_ougoing_predicate: '+str(p)+'\tobject:'+str(o))
            total_score = 0
            for k, sims in target_outgoing_sims.items():
                max_sim = 0
                for cand_out_ent in candidate_outgoing[k]:
                    if cand_out_ent in self.e2i.keys():
                        for ent, sim in sims.items():
                            max_sim = max(max_sim, sim_weight[k][ent]*sim[0, self.e2i[cand_out_ent]])
                            print('cand_out_ent:'+str(cand_out_ent)+'\tent: '+str(ent)+'\tsim: '+str(sim))
                total_score += max_sim
                candidate_score_breakdown[c].append(max_sim)
            best_outgoing_sim = 0
            for outgoing_forecast_ent in self.KG.objects(subject=c, predicate=forecast_step_relation):
                ...
                outgoing_relations = set(self.triple_dict_forward[outgoing_forecast_ent].keys())
                print('outgoing_forecast_ent: '+str(outgoing_forecast_ent))
                print('outgoing_relations: '+str(outgoing_relations))
                # get jaccard sim
        ...
        sorted_candidates = [tup[0] for tup in sorted_candidates]
        print('================================')
        print('Output information with coverage')
        print('=================================')
        print('Sorted_candidates: '+str(sorted_candidates))
        print('sc_scores: '+str(sc_scores))
        print('case_connection_scores:'+str(case_connection_scores))
    ...
    def collect_similar_events_with_target_relation(self, dummy_target_uri: URIRef, target_relation: URIRef, initial_candidate_limit: int = None) -> \
        ...
        print('============================')
        print('Dummy target relation')
        print(dummy_target_relations)
        print('=============================')
        dummy_target_vec = np.zeros((1, len(self.r2i.keys())))
            ...
            for prop, paths in prop_paths.items():
                ...
                for path in paths:
                    ...
                    for ind, e in enumerate(path):
                        ...
                        print("#################")
                        print("E: "+str(e))
                        print("Edge Triple ALL: "+str(edge_triple_all))
                        print("####################")
                        next_paths = []
                        for edge_triple in edge_triple_all:
                            if edge_triple[0] != prev_node:
                ...
                miniprop_totals = defaultdict(lambda: 0)
                print("#################################")
                print("ALL PATH EDgEs: "+str(all_path_edges))
                print("##############################")
                for path_edges in all_path_edges:
                    miniprop_counts[path_edges] += 1
                    ft = time.time()
