...
class EvCBR:
    ...
    ###################################
    def refine_with_ws(self,precomp_sim_cases_refined_EFF,case_connection_scores_EFF):
      selected_cases = []
      for sim_case in precomp_sim_cases_refined_EFF:
        selected_cause = False
        who_cause_flag = False
        where_cause_flag = False
        for p,o in self.KG.predicate_objects(subject=URIRef(sim_case)):
          if("hasActor" in str(p)):who_cause_flag=True
          if("hasPlace" in str(p)):where_cause_flag=True
        if(who_cause_flag == True and where_cause_flag ==True): 
          selected_cause = True
          #break
        connected_effects = case_connection_scores_EFF[sim_case]
        if(len(connected_effects)==0):
          continue
        if(selected_cause == True):
          who_effect_flag = False
          where_effect_flag = False
          for p,o in self.KG.predicate_objects(subject=URIRef(sim_case)):
            if("hasActor" in str(p)):who_cause_flag=True
            if("hasPlace" in str(p)):where_cause_flag=True
          if(who_effect_flag == True and where_effect_flag ==True): 
            #selected_cause = True
            selected_cases.append(sim_case)
