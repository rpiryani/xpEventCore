...
def vectorize_graph(in_dir: Path, out_dir: Path, do_outgoing_edges: bool = False,  do_two_hop: bool = False, do_superclasses: List[URIRef] = None):
    ...
    if do_outgoing_edges:
        ...
        for ent in e2i.keys():
            ...
            for s in all_super:
                subclass_matrix[e2i[ent], s] = 1
                print("E2i[ent]"+str(e2i[ent])+"\ts="+str(s)+"\t")
                subclass_counts[s] += 1
    ...
    for i in range(subclass_counts.shape[0]):
        if subclass_counts[i] != 0:
            ...
            print("subclass_counts[i]="+str(subclass_counts[i])+"\ti="+str(i)+"\t subclass_totals:"+str(subclass_totals))
...
def main(args):
    ...
    ############
    if args.process_emvista:
        emvista_input_dir = (DATA_DIR / args.emvista_input).resolve()
        emvista_output_dir = (DATA_DIR / args.emvista_output).resolve()
        if not os.path.isdir(emvista_output_dir):
            os.mkdir(emvista_output_dir)
    ############
    ...
    #########################
    if args.process_emvista:
        print("starting Emvista preprocessing")
        fg, fgi = vectorize_graph(in_dir=emvista_input_dir, out_dir=emvista_output_dir, do_outgoing_edges=True, do_two_hop=True)
    else:
        print("skipping Emvista")
    #########################
    if args.process_wiki:
...
if __name__ == "__main__":
    ...
    #=======
    parser.add_argument("--process_emvista",action='store_true', default=False)
    #==========
    ...
    #=======
    parser.add_argument("--emvista_input", type=str, default="pp_emvista")
    parser.add_argument("--emvista_output", type=str, default="evcbr_pp_emvista")
    #==========
