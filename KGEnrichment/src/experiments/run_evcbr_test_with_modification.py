...
def naive_evaluation_single_sample(F: EvCBR, q, output,ranked_list_xp_event,
    ...
    while True:
        ...
        print("----------------------")
        print("s_p_o and truth_pred_objs")
        print("s_p_o")
        print(s_p_o)
        print("truth_pred_objs")
        print(truth_pred_objs)
        print("---------------------------")
        ...
        print("----------------------")
        print("Forecase Properties")
        print(forecast_properties)
        print("----------------------")
        ...
        print("-----------------------")
        print("dummy_target_uri:"+eval_s)
        print("target_relation: "+eval_p)
        print("Precomp_sim_cases")
        print(precomp_sim_cases)
        print("-----------------------")
        precomp_sim_cases_refined, pscs, case_connection_scores = F.refine_similar_cases_with_target_forecasts(
            ...
            print_info=False)
        print("-----------------------")
        print("precomp_sim_cases_refined:")
        print(precomp_sim_cases_refined)
        print("pscs")
        print(pscs)
        print("case_connection_scores")
        print(precomp_sim_cases)
        print("-----------------------")
        ...
        temp_s_cases = F.refine_with_ws(precomp_sim_cases_refined,case_connection_scores)
        print("LENGTH TEMP S_CASES: "+str(len(temp_s_cases)))
        print("temp_s_cases: "+str(temp_s_cases))
        for sim_case in temp_s_cases:
          connected_effects = case_connection_scores[sim_case]
          print("SimCase: "+str(sim_case))
          print("case_connection_scores_EFF[sim_case]: "+str(case_connection_scores[sim_case]))
          print("Connected_effects: "+str(connected_effects))
          print("Connected_effects values: "+str(connected_effects.values()))
          
          #break
        precomp_sim_cases_refined = temp_s_cases
        ...
        precomp_sim_cases_refined_EFF, pscs_EFF, case_connection_scores_EFF = F.refine_similar_cases_with_target_forecasts_effect_coverage(
            ...
            print_info=False)
        ####################################################
        temp = F.refine_with_ws(precomp_sim_cases_refined_EFF,case_connection_scores_EFF)
        print("LENGTH TEMP : "+str(len(temp)))
        print("temp: "+str(temp))
        print("TEMP: "+str(temp))
        for sim_case in temp:
          print("SimCase: "+str(sim_case))
          print("case_connection_scores_EFF[sim_case]: "+str(case_connection_scores_EFF[sim_case]))
          print("connected_effects.values():"+str(connected_effects.values()))
          
        #####################################################
        ###################################################
        print("#######################################")
        print("FINAL LENGTH: "+str(len(precomp_similar_cases_final)))
        #####################################################
        if(len(temp)<s_cases_cov):
          print("##################YES#######################")
          counter = 0
          for sim_case in precomp_sim_cases_refined_EFF:
            if(URIRef(sim_case) not in temp):
              temp.append(sim_case)
              counter+=1
            if(counter==(s_cases_cov-len(temp)-1)):break
        precomp_sim_cases_refined_EFF = temp    
        for ind, sim_case in enumerate(precomp_sim_cases_refined_EFF[:s_cases_cov]):
            ...
            if(len(connected_effects)==0):
              print("CAUSE EMPTY: "+str(sim_case))
              print("Connected_effects EMPTY: "+str(connected_effects))
              continue
            max_effect_score = max(connected_effects.values())
            best_options = [k for k,v in connected_effects.items() if v==max_effect_score]
            if len(best_options) > 1:
                ...
                connection_choice = best_options[0]
            precomp_similar_cases_final.append((sim_case, connection_choice))
            #print("INDEX: "+str(len(ind)))
        ######################################
        print('#######################################')
        print("TEST###################################")
        print("ScaseCOverage = "+str(s_cases_cov))
        print("#######################################")
        for sim_case,connection_choice in precomp_similar_cases_final:
          print("Sim_case: "+str(sim_case))
          print("Connection_choice: "+str(connection_choice))
          break
        print("LENGTH BEFORE FINAL: "+str(len(precomp_sim_cases_refined_EFF)))
        print("LENGTH OF FINAL: "+str(len(precomp_similar_cases_final)))
        print("CASE CONNECTION SCORE: "+str(case_connection_scores_EFF))
        print("PRECOMPUTED SIMMILAR CASES: "+str(precomp_sim_cases_refined_EFF))
        print("PRECOMPUTED SIMMILAR CASES LEN: "+str(len(precomp_sim_cases_refined_EFF)))
        print("PRECOMPUTED SIMMILAR CASES TEMP: "+str(temp))
        print("PRECOMPUTED SIMMILAR CASES TEMP LEN: "+str(len(temp)))
        print("#######################################")
        ######################################
        ##########################
        print("----------------------------")
        print("Precomp_similar_cases_final")
        print(precomp_similar_cases_final)
        print("---------------------------")
        ##########################
        no_fwd_preds = 0
        ...
        for ep_ind, (target_rel, target_truth) in enumerate(truth_pred_objs):
            ...
            print('------------------------------------')
            print('ep_ind: '+str(ep_ind))
            print('target_rel:'+str(target_rel))
            print('target_truth:'+str(target_truth))
            print('rank:'+str(rank))
            print('sorted_forecast_res:')
            print(sorted_forecast_res)
            print('------------------------------------')
            #output[(eval_s,eval_p,eval_o)] = {"cause_uri": eval_s, "effect_uri": eval_o, "forecast_property_ranks": this_ranks}
            ranked_list_xp_event.append({'eval_s':eval_s,'eval_p':eval_p,'eval_o':eval_o,'target_rel':target_rel,'target_truth':target_truth,"rank":rank,'Sorted_forecast_res':sorted_forecast_res})
            
        ##########################
        print("-------------------------")
        print("sorted_forecast_res Effects")
        print(sorted_forecast_res)
        print("---------------------------")
        ##########################
        ...
        print({"cause_uri": eval_s, "effect_uri": eval_o,
                              "forecast_property_ranks": this_ranks,
                              "property_combos": reverse_combos,
                                              })
        print("-----------------------------")
...
def main(args):
    ...
    pool = mp.Pool(args.processes, initializer=naive_evaluation_single_sample, initargs=(
        F, q, output_dict,ranked_list_xp_event,
        args,
    ...
    with open((save_dir / "eval_res.pkl").resolve(), "wb") as f:
        pickle.dump(dict(output_dict), f)
    #ranked_list_xp_event_df = pd.DataFrame(list(ranked_list_xp_event))
    #with open((save_dir / "ranked_list.pkl").resolve(), "wb") as f:
        #pickle.dump(dict(ranked_list_xp_event), f)
    #json_data = json.dumps(list(ranked_list_xp_event))
    with open((save_dir / "ranked_list.json").resolve(), "w") as f:
        json.dump(list(ranked_list_xp_event), f,indent=4)
    
    #excel_writer = pd.ExcelWriter((save_dir / "ranked_list.xlsx").resolve(), engine='xlsxwriter')
    #ranked_list_xp_event_df.to_excel(excel_writer, sheet_name='Sheet1', index=False)
    #excel_writer.save()
